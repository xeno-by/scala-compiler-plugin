...
object dataMacro {
  ...
  // 7) No referential transparency for methods like apply or copy that are injected into the companion, but should use data class definition scope

  abstract class dataHelper[C <: Context](val c: C) {
    import c.universe._
    ...
    def makeCaseAccessor(mods: Modifiers) = {
      if (isByName(mods)) c.abort(c.enclosingPosition, "`data' parameters may not be call-by-name")
  ...
  class dataClassHelper[C <: Context](override val c: C) extends dataHelper(c) {
    import c.universe._
    ...
    def expand(annottees: List[c.Tree]): List[c.Tree] = {
      ...
      if (primaryCtor.vparamss.isEmpty) c.abort(c.enclosingPosition, "`data' is not appplicable to classes without a parameter list")
      ...
      val tparamUnderscores = tparams.zipWithIndex.map { case (tdef, i) => TypeDef(unmakeVariant(makeDeferredSynthetic(unmakeParam(tdef.mods))), TypeName("x$" + (i + 1)), tdef.tparams, tdef.rhs) }
      ...
      val dataClass = {
        ...
        val cbody3 = {
          if (cbody2.collect { case ddef @ DefDef(_, name, _, _, _, _) if name == TermName("copy") => ddef }.nonEmpty) cbody2
            ...
            val copyTparams = tparams.map(tdef => TypeDef(unmakeVariant(tdef.mods), tdef.name, tdef.tparams, tdef.rhs))
      ...
      List(dataClass, dataModule)
    }
  }

  class dataTraitHelper[C <: Context](override val c: C) extends dataHelper(c) {
    import c.universe._
    import definitions._
    ...
    def expand(annottees: List[c.Tree]): List[c.Tree] = {
      ...
      val mdef @ ModuleDef(mmods, mname, Template(mparents, mself, mbody)) = annottees.tail.headOption.getOrElse {
        val emptyCtor = DefDef(Modifiers(), termNames.CONSTRUCTOR, List(), List(List()), TypeTree(), Block(List(Apply(Select(Super(This(typeNames.EMPTY), typeNames.EMPTY), termNames.CONSTRUCTOR), List())), Literal(Constant(()))))
        ModuleDef(SyntheticMods, name.toTermName, Template(List(Ident(AnyRefClass)), noSelfType, List(emptyCtor)))
      }
      List(cdef, mdef)
    }
  }

  class dataObjectHelper[C <: Context](override val c: C) extends dataHelper(c) {
    import c.universe._
  ...
  def impl(c: Context)(annottees: c.Expr[Any]*): c.Expr[Any] = {
    ...
    val helper = {
      annottees.head.tree match {
        case tree @ ClassDef(mods, _, _, _) if !mods.hasFlag(PublicFlags.TRAIT) => new dataClassHelper[c.type](c)
        case tree @ ClassDef(mods, _, _, _) if mods.hasFlag(PublicFlags.TRAIT) => new dataTraitHelper[c.type](c)
        case tree @ ModuleDef(_, _, _) => new dataObjectHelper[c.type](c)
        case _ => c.abort(c.enclosingPosition, "`data' is only applicable to classes and objects")
      }
    }
